...
"""
The model-checker provides a countermodel to the claim that Diamond(A) does not entail Box(A), which makes sense since possibility is a much weaker
constraint than necessity. One example in English is: "It is possible that it is sunny outside", which does not entail "It is necessary that it is sunny outside."

The model-checker gives an example where Diamond A is true by virtue of the base world accessing two worlds, W_0 and W_1, where A is false at W_0 (thus not necessary), but A is true at W_1 (thus possible).
"""
...
# MD_CM_7: McKinsey Axiom
MD_CM_7_premises = ['\\Box \\Diamond A']
MD_CM_7_conclusions = ['\\Diamond \\Box A']
MD_CM_7_settings = {
    'N' : 1,
    'M' : 1,
    'contingent' : False,
...
"""
The model-checker provides a countermodel to the McKinsey axiom, showing that Box(Diamond(A)) does not imply Diamond(Box(A)). The invalidity of this inference
is (half) the reason that the Box and Diamond operators don't commute with each other.

In English, a concrete example is "It must be the case that I possibly rolled a 6 with the die" (which sounds plausible), which does not imply
"It is possibly the case that I must have rolled a 6" (which sounds implausible).
"""
...
# BM_TH_5: 'Could-never-have-been' implies 'wasn't'
BM_TH_5_premises = ['\\leftbroom A']
BM_TH_5_conclusions = ['\\past \\neg A']
BM_TH_5_settings = {
    'N' : 1,
    'M' : 2,
    'contingent' : False,
