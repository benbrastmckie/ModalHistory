...
class CouldNeverHaveBeenOperator(syntactic.DefinedOperator):
    """Bimodal operator that returns true iff at the current time, all world-time pairs in the past had not-Diamond(A).
    
    This operator represents the notion of "as it turns out, we needn't have worried about P occurring". Specifically, for all times in this worlds past,
    it wasn't possible that P, and so now, 'it could never have been that P'
        
    Example: Upon learning that a (previously-believed fair) dice is marked so that all sides are 2,4,6, one might say "As it turns out, it could never have 
    been the case that I rolled an odd number".

    Note that we implement (past(not-possible)) as (past(necessarily-not)), as these are equivalent, and 'necessarily' is already defined.    
    """
    ...
    def derived_definition(self, argument):  # type: ignore
        return [PastOperator, [NecessityOperator, [NegationOperator, argument]]]
