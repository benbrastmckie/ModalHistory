...
class SometimesOperator(syntactic.Operator):
    """Temporal operator that evaluates whether a formula holds at some point in the past, present, or future.

    This operator implements 'it is sometimes the case that'. It evaluates whether its argument holds at some time
    within the eval_world 

    Key Properties: 
    Returns true if the argument is true at some time in the history of a world. 
    Returns false if it is not the case that it was true at some point in the past, present, or future. 
    --intuitively, if a formula is true in the bimodal semantics, it is sometimes true. This is because specifying the truth of a formula requires specifying some time in which it is true.  

    Methods:
        true_at: Returns true if argument holds at any time in a history
        false_at: Returns true if argument never holds in a history 
        find_truth_condition: Computes temporal profiles for all worlds
        print_method: Displays evaluation across different time points

    Example:
        If p means "it's raining", then ∇p  could means "it rains sometimes"
    """
    name = "\\Sometimes"
    arity = 1

    def true_at(self, argument, eval_world, eval_time):
        """Returns true if argument is true at some time in this world's interval
        ...
        #gets past, current, and future times 
        past_time = z3.Int("past_true_time")
        current_time=z3.Int("eval_time")
        future_time = z3.Int('future_true_time')
        total_time=past_time or current_time or future_time 
        total_time_2=past_time and current_time and future_time

        return z3.ForAll(
            past_time and current_time and future_time,
            z3.Implies(
                z3.And(
                    # Time is within the valid range for this world's interval
                    semantics.is_valid_time_for_world(eval_world, total_time_2),
                ),
                semantics.true_at(argument, eval_world, total_time),
            )
        )
    
    def false_at(self, argument, eval_world, eval_time):
        """Returns true if is not true at any time in this world's interval.
        ...
        return z3.Exists(
            ...
            z3.And(
                ...
                semantics.false_at(argument, eval_world, total_time),
            )
        )
