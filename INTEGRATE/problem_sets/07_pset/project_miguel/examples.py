...
"""
The model-checker provides a countermodel to this claim, which makes sense since what will
always be true need not necessarily be true. 

In the countermodel, A is true at world state c and not true at world state d. In the world
of evaluation at the time of evaluation, the world state is d, so A is false. However at the
next time, the world state is true, so A is true then. So \\Future A is true at the time of
evaluation. And on the task semantics just by the fact that A is not currently true \\Box A
comes out as false, but regardless there is another world in which A is not true at the evaluation
time, which (even on a Kaplanian bimodal semantics) makes \\Box A come out as false at the
world and time of evaluation. 
"""
...
# NEW_EXAMPLE: MY OWN EXAMPLE
BM_NEW_EXAMPLE_premises = ['\\Diamond \\Past A']
BM_NEW_EXAMPLE_conclusions = ['\\Box \\Past A']
BM_NEW_EXAMPLE_settings = {
    'N' : 2,
    'M' : 2,
    'contingent' : False,
    'disjoint' : False,
...
BM_NEW_EXAMPLE_example = [
    BM_NEW_EXAMPLE_premises,
    BM_NEW_EXAMPLE_conclusions,
    BM_NEW_EXAMPLE_settings,
]
"""
The model-checker times out at 30 seconds unfortunately with N=2 and M=2. It also times out
with \\Future instead of \\Past. 
"""
...
# DORR_AND_GOODMAN: NECESSARY IS ALWAYS
DAG_premises = ['\\Box A']
DAG_conclusions = ['\\Always A']
DAG_settings = {
    'N' : 2,
    'M' : 2,
    'contingent' : False,
...
example_range = {
    ...
    # Modal Theorems
    # "MD_TH_1" : MD_TH_1_example,
    # "MD_TH_2" : MD_TH_2_example, # correctly found no countermodel
    ...
    # Bimodal Theorems
    # "BM_TH_1" : BM_TH_1_example,
    # "BM_TH_2" : BM_TH_2_example, # correctly found no countermodel
    # "BM_TH_3" : BM_TH_3_example,
    # "BM_TH_4" : BM_TH_4_example,

    # My own examples
    # "BM_NEW_EXAMPLE": BM_NEW_EXAMPLE_example # times out at 30 seconds
    "Dorr_and_Goodman": DAG_example
}
