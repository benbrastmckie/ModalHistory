...
"""
The example says roughly that "if A grounds B, then (A or B) is equivalent to B". Of course, if B, then (A or B). As A grounds B, the verifiers of 'A or B' are just the verifiers of B. The falsifiers of 'A or B' are those falsifiers of A and of B. But as A grounds B, every falsifier of B has a part that falsifies A. So the falsifiers of A and of B are just the falsifiers of B (which subsequently have a part which falsify A). Thus 'A or B' has the same verifiers and falsifiers as 'B', thus they are equivalent.

Concrete example: If our premise is " 'it is raining and not nighttime' grounds that 'it is raining' ", then we conclude that "it is raining and not nighttime, OR it is raining" is in fact equivalent to "it is raining". 
"""
...
# CL_TH_9: 
CL_TH_9_premises = ['(A \\boxright B)']
CL_TH_9_conclusions = ['(A \\leq B)']
CL_TH_9_settings = {
    'N' : 1,
    'contingent' : False,
    'disjoint' : False,
...
"""
The claim above is that 'if A were the case, B would be the case' implies that 'A entails B'. This seems straightforwardly false; A implying B in some nearby world does not imply that A entails B in general.

The model-checker successfully finds a countermodel; it gives one single state space, the null state. It treats 'A' as being verified at the null state, and falsified nowhere, and similarly for B. Then as the nearest A-alternative to the current state (the null state) is still the null state, we have A boxright B, as both are true at the nullstate. 

However I am a bit confused on the conclusion. The model-checker claims 'A entails B' to be false, but shows A and B as being equivalent, since they have the same verifiers and falsifiers. But then it sounds reasonable to infer 'A entails A'.
"""

# CL_TH_10: Example 10
CL_TH_10_premises = []
CL_TH_10_conclusions = ['(\\CFBox A \\leftrightarrow \\Box A)']
CL_TH_10_settings = {
    'N' : 7,
    'contingent' : False,
    'disjoint' : False,
...
"""
This example seems to time out for my computer around N=8, although despite the longer wait time, it reads as runtime =~ 1 second for both N=6 and N=7.
"""

# CL_TH_11: Example 11
CL_TH_11_premises = []
CL_TH_11_conclusions = ['(\\CFDiamond A \\leftrightarrow \\neg \\CFBox \\neg A)']
CL_TH_11_settings = {
    'N' : 7,
    'contingent' : False,
    'disjoint' : False,
...
"""
Similar to example 10, the model-checker fails to find countermodels for higher values of N, up to N=7, where the runtime is capped at 1 but it takes longer for the computer to register this. 
"""

# CL_TH_12: Example 12
CL_TH_12_premises = []
CL_TH_12_conclusions = ['(\\Diamond A \\leftrightarrow \\neg \\Box \\neg A)']
CL_TH_12_settings = {
    'N' : 9,
    'contingent' : False,
    'disjoint' : False,
...
"""
While this example 12 can be seen to be valid since it is just the defintion of the diamond (or box) operator, the model-checker has a much easier time handling
this sentence, and can run in half the time while considering more atomic states than either examples 10 or 1l, which contain the counterfactual operators.
"""
#########################################
